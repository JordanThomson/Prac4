...
uint32_t saw_LUT[NS] = {0,8,16,24,32,40,48,56,64,72,80,
		88,96,104,112,120,128,136,144,153,161,169,177,185,193,201,209,
		217,225,233,241,249,257,265,273,281,289,298,306,314,322,330,338,
		346,354,362,370,378,386,394,402,410,418,426,434,443,451,459,467,475,
		483,491,499,507,515,523,531,539,547,555,563,571,579,588,596,604,612,
		620,628,636,644,652,660,668,676,684,692,700,708,716,724,733,741,749,
		757,765,773,781,789,797,805,813,821,829,837,845,853,861,869,878,886,
		894,902,910,918,926,934,942,950,958,966,974,982,990,998,1006,1014,1023};
...
WaveformType current_waveform = SINE_WAVE;
...
void EXTI0_1_IRQHandler(void)
{
	...
	// Add a debounce delay of 200 milliseconds
	if (current_time - last_button_press_time < 200)
	{
	   // Ignore button presses too close to each other (debounce)
		return;
	}
	...
	// Switch between waveforms (LUTs)
	    switch (current_waveform)
	    {
	        ...
	        default:
	            // Handle an unknown waveform (if needed)
	            break;
	    }
